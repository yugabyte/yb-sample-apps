 ...
 * they were specified, print help messages when running the app, etc.
 */
public class CmdLineOpts {
	...
	private final static List<Class> HELP_WORKLOADS = ImmutableList.of(CassandraHelloWorld.class,
			CassandraKeyValue.class, CassandraRangeKeyValue.class, CassandraBatchKeyValue.class,
			CassandraBatchTimeseries.class, CassandraEventData.class, CassandraTransactionalKeyValue.class,
			CassandraTransactionalRestartRead.class, CassandraStockTicker.class, CassandraTimeseries.class,
			CassandraUserId.class, CassandraPersonalization.class, CassandraSecondaryIndex.class,
			CassandraUniqueSecondaryIndex.class, RedisKeyValue.class, RedisPipelinedKeyValue.class,
			RedisHashPipelined.class, RedisYBClientKeyValue.class, SqlInserts.class, SqlUpdates.class,
			SqlSecondaryIndex.class, SqlSnapshotTxns.class);
	...
	public void initialize(CommandLine commandLine) throws ClassNotFoundException {
		...
		if (appName.equals(CassandraBatchTimeseries.class.getSimpleName())) {
			if (commandLine.hasOption("read_batch_size")) {
				...
				LOG.info("CassandraBatchTimeseries batch size: " + AppBase.appConfig.cassandraReadBatchSize);
			}
			if (commandLine.hasOption("read_back_delta_from_now")) {
				...
				LOG.info("CassandraBatchTimeseries delta read: " + AppBase.appConfig.readBackDeltaTimeFromNow);
			}
		}
		...
		if (commandLine.hasOption("batch_size")) {
			AppBase.appConfig.cassandraBatchSize = Integer.parseInt(commandLine.getOptionValue("batch_size"));
		...
		if (appName.equals(CassandraPersonalization.class.getSimpleName())) {
			...
			if (AppBase.appConfig.numNewCouponsPerCustomer > AppBase.appConfig.maxCouponsPerCustomer) {
				LOG.fatal("The number of new coupons cannot exceed the maximum number of coupons per customer");
				System.exit(-1);
			}
		...
		if (appName.equals(RedisHashPipelined.class.getSimpleName())) {
			if (commandLine.hasOption("num_subkeys_per_key")) {
				AppBase.appConfig.numSubkeysPerKey = Integer
						.parseInt(commandLine.getOptionValue("num_subkeys_per_key"));
				if (AppBase.appConfig.redisPipelineLength > AppBase.appConfig.numUniqueKeysToWrite) {
					LOG.fatal("The pipeline length cannot be more than the number of unique keys");
					System.exit(-1);
				}
			...
			if (commandLine.hasOption("key_freq_zipf_exponent")) {
				AppBase.appConfig.keyUpdateFreqZipfExponent = Double
						.parseDouble(commandLine.getOptionValue("key_freq_zipf_exponent"));
			}
			if (commandLine.hasOption("subkey_freq_zipf_exponent")) {
				AppBase.appConfig.subkeyUpdateFreqZipfExponent = Double
						.parseDouble(commandLine.getOptionValue("subkey_freq_zipf_exponent"));
			}
			if (commandLine.hasOption("subkey_value_size_zipf_exponent")) {
				AppBase.appConfig.valueSizeZipfExponent = Double
						.parseDouble(commandLine.getOptionValue("subkey_value_size_zipf_exponent"));
			}
			if (commandLine.hasOption("subkey_value_max_size")) {
				AppBase.appConfig.maxValueSize = Integer.parseInt(commandLine.getOptionValue("subkey_value_max_size"));
			}
			if (commandLine.hasOption("num_subkeys_per_write")) {
				AppBase.appConfig.numSubkeysPerWrite = Integer
						.parseInt(commandLine.getOptionValue("num_subkeys_per_write"));
			...
			if (commandLine.hasOption("num_subkeys_per_read")) {
				AppBase.appConfig.numSubkeysPerRead = Integer
						.parseInt(commandLine.getOptionValue("num_subkeys_per_read"));
		...
		if (commandLine.hasOption("concurrent_clients")) {
			AppBase.appConfig.concurrentClients = Integer.parseInt(commandLine.getOptionValue("concurrent_clients"));
		}
	...
	private static Class<? extends AppBase> getAppClass(String workloadType) throws ClassNotFoundException {
		// Get the workload class.
		return Class.forName("com.yugabyte.sample.apps." + workloadType).asSubclass(AppBase.class);
	}
	...
	private void initializeThreadCount(CommandLine cmd) {
		...
		LOG.info("Num reader threads: " + numReaderThreads + ", num writer threads: " + numWriterThreads);
	}
	...
	private void initializeNumKeys(CommandLine cmd) {
		...
		AppBase.appConfig.maxWrittenKey = Long
				.parseLong(cmd.getOptionValue("max_written_key", String.valueOf(AppBase.appConfig.maxWrittenKey)));
	...
	public static CmdLineOpts createFromArgs(String[] args) throws Exception {
		...
		options.addOption("nouuid", false, "Do not use a UUID. Keys will be key:1, key:2, key:3, "
		...
		options.addOption("read_only", false,
				"Read-only workload. " + "Values must have been written previously and uuid must be provided. "
		...
		options.addOption("sleep_time", true, "How long (in ms) to sleep between multiple pipeline batches.");
		options.addOption("socket_timeout", true, "How long (in ms) to wait for a response from jedis.");
		options.addOption("value_size", true,
		...
		options.addOption("use_ascii_values", false,
				"[RedisKeyValue] If " + "specified, values are restricted to ASCII strings.");
		...
		options.addOption("disable_yb_load_balancing_policy", false, "Disable Yugabyte load-balancing policy.");
		...
		options.addOption("run_time", true, "Run time for workload. Negative value means forever (default).");
		options.addOption("use_redis_cluster", false, "Use redis cluster client.");
		options.addOption("yql_username", true, "Use authentication with the YQL client using the provided username. "
				+ "If this option is set, yql_password option should be used too.");
		options.addOption("yql_password", true, "Use authentication with the YQL client using the provided password. "
		...
		options.addOption("ssl_cert", true, "Use an SSL connection while connecting to YugaByte.");

		// Options for CassandraTimeseries workload.
		options.addOption("num_users", true, "[CassandraTimeseries] The total number of users.");
		options.addOption("min_nodes_per_user", true, "[CassandraTimeseries] The min number of nodes per user.");
		options.addOption("min_nodes_per_user", true, "[CassandraTimeseries] The min number of nodes per user.");
		options.addOption("max_nodes_per_user", true, "[CassandraTimeseries] The max number of nodes per user.");
		...
		options.addOption("num_unique_keys", true, "[KV workloads only] Number of unique keys to write into the DB.");
		...
		options.addOption("batch_size", true, "[CassandraBatchKeyValue] Number of keys to write in a batch.");

		// Options for CassandraBatchTimeseries app.
		options.addOption("read_batch_size", true, "[CassandraBatchTimeseries] Number of keys to read in a batch.");
		...
		options.addOption("num_stores", true, "[CassandraPersonalization] Number of stores.");
		...
		options.addOption("non_transactional_index", false,
				"[CassandraSecondaryIndex] Create secondary index without transactions " + "enabled.");
		options.addOption("batch_write", false, "[CassandraSecondaryIndex] Enable batch write of key values.");

		// Options for Redis Pipelined Key Value
		options.addOption("pipeline_length", true,
				"[RedisPipelinedKeyValue/RedisHashPipelined] Number of commands to be sent out"
						+ " in a redis pipelined sync.");

		options.addOption("num_subkeys_per_key", true,
				"[RedisHashPipelined] Number of subkeys in each key for the RedisHashPipelined workload.");
		options.addOption("num_subkeys_per_write", true, "[RedisHashPipelined] Number of subkeys updated in each HMSet "
				+ "for the RedisHashPipelined workload.");
		options.addOption("num_subkeys_per_read", true,
				"[RedisHashPipelined] Number of subkeys read in each HMGet " + "for the RedisHashPipelined workload.");

		options.addOption("key_freq_zipf_exponent", true,
				"[RedisHashPipelined] The zipf distribution exponent, if keys "
						+ "should be picked using a Zipf distribution. If <= 0, we use " + "a uniform distribution");
		options.addOption("subkey_freq_zipf_exponent", true,
				"[RedisHashPipelined] The zipf distribution exponent, if subkeys "
						+ "should be picked using a Zipf distribution. If <= 0, we use " + "a uniform distribution");
		options.addOption("subkey_value_size_zipf_exponent", true,
		...
		options.addOption("subkey_value_max_size", true,
	 ...
	 * Kubernetes yb-tservers.default.svc.cluster.local contact point is resolved to
	 * all tservers IPs.
	 */
