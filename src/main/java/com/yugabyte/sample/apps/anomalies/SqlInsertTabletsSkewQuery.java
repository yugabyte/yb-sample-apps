...
public class SqlInsertTabletsSkewQuery extends SqlInsertTablets {
  ...
  private static final AtomicInteger READ_SKEW_SETUP = new AtomicInteger(0);
  private static final AtomicInteger WRITE_SKEW_SETUP = new AtomicInteger(0);
  ...
  public void setupReadThread() {
    if (READ_SKEW_SETUP.get() > 10) return;
    if (READ_SKEW_SETUP.incrementAndGet() == 1) {
  ...
  public void setupWriteThread(int threadIdx) {
    if (WRITE_SKEW_SETUP.get() > 10) return;
    if (WRITE_SKEW_SETUP.incrementAndGet() == 1) {
      LOG.info(
